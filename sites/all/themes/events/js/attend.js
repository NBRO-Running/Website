((function($) {
	 ...
	 * particle array...
	 */
	var initCanvas = function(element) {
	...
	console.log('Canvas height', h);
	 ...
	 source = {
	    'x': round(w / 2),
	    'y': element.getBoundingClientRect().top + (window.pageYOffset || doc.scrollTop)  - (doc.clientTop || 0)
	  };
	...
	var drawOnCanvas = function() {
	  ...
	  for (var i = 0; i < N_PARTICLES; i++) {
	    ...
	    } else { //ource.y = document.getElementById('slider').getBoundingClientRect().top;if not...
	...
	$('strong.fb-attend-slider').each(function() {
			var slider = $(this);
			slider.click( function(e) {
				...
				var offset = 0;
				if(well.hasClass('attending')){
				slider.animate({left: 0}, {duration: 750, complete: function() {
					$.post("https://graph.facebook.com/"+well.attr('id')+"/declined", {access_token : Drupal.settings.events.facebook_auth});
					well.find('span').html("PRESS TO ATTEND");
					well.removeClass('attending');
					}
					});
				}else {
					$('canvas.c').width($(window).width());
					$('canvas.c').height(1200);
					initCanvas(slider[0]);
					$('canvas.c').show();
					window.ANIMATION_FINISHED = false;
					slider.animate({left: wellWidth - imageWidth + offset}, {duration: 750, step: function(now, fx) {
					ctx.clearRect(0, 0, w, h);
					source.x = slider.offset().left + slider.width() / 2;
					}, complete: function() {
					//Post attend to FB
					$.post("https://graph.facebook.com/"+well.attr('id')+"/attending", {access_token : Drupal.settings.events.facebook_auth});
					window.ANIMATION_FINISHED = true;
					well.addClass('attending');
